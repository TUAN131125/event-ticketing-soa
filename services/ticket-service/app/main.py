"""FastAPI entrypoint for Ticket Service."""
...
from app.infrastructure.database.repositories import ticket_counts_by_status
...
from app.observability.metrics import TICKETS_BY_STATUS

LOGGER = logging.getLogger("ticket.metrics")
STATUS_NAMES = ("VALID", "CHECKED_IN", "CANCELLED")
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    application = FastAPI(
        ...
        version=__version__,
        description=(
            "Internal digital ticket issuance, versioned QR and venue check-in API."
        ),
    ...
    def metrics() -> Response:
        try:
            service: TicketService = application.state.ticket_service
            with service.session_factory() as session:
                with session.begin():
                    prepare_transaction(session, current)
                    for status_name in STATUS_NAMES:
                        TICKETS_BY_STATUS.labels(status_name).set(0)
                    for status_name, count in ticket_counts_by_status(session):
                        TICKETS_BY_STATUS.labels(status_name).set(count)
        except Exception:
            LOGGER.warning("Unable to refresh ticket status metrics", exc_info=True)
