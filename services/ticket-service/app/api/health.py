...
def create_health_router(settings: Settings) -> APIRouter:
    ...
    @router.get(
        "/health/live", operation_id="ticketLiveness", response_model=HealthStatus
    )
    async def live() -> HealthStatus:
        return HealthStatus(service=settings.app_name, status="UP", version=__version__)

    @router.get(
        "/health/ready", operation_id="ticketReadiness", response_model=HealthStatus
    )
    async def ready(request: Request) -> JSONResponse:
        if request.app.state.draining:
            ...
            return JSONResponse(
                error_envelope(
                    ...
                    message="Ticket service is draining",
                    retryable=True,
                ),
                status_code=503,
            )
        ...
        return JSONResponse(
            error_envelope(
                request,
                code="SERVICE_UNAVAILABLE",
                message="Ticket database is not ready",
                retryable=True,
            ),
            status_code=503,
        )
