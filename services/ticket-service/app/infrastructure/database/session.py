    ...
    try:
        with get_engine(settings).connect() as connection:
            row = connection.execute(
                text(
                    "SELECT to_regclass('ticket.tickets'), "
                    "to_regclass('ticket.idempotency_records'), "
                    "to_regclass('ticket.ticket_audit'), "
                    "to_regclass('ticket.outbox_events'), "
                    "to_regclass('ticket.alembic_version')"
                )
            ).one()
