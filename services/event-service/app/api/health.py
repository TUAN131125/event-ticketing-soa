...
@router.get("/health/live", operation_id="eventLiveness", response_model=HealthStatus)
def liveness() -> HealthStatus:
    return HealthStatus(status="UP")


@router.get("/health/ready", operation_id="eventReadiness", response_model=HealthStatus)
def readiness(request: Request) -> Response:
    ...
    return JSONResponse(
        error_envelope(
            ...
            message="Event database is not ready",
            retryable=True,
        ),
        status_code=503,
    )
