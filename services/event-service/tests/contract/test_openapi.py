...
def test_provider_matches_canonical_contract() -> None:
    settings = Settings(
        ...
        database_url="postgresql+psycopg://event:test@localhost/event",
        db_pool_size=1,
        db_max_overflow=0,
        sql_echo=False,
        service_jwt=make_service_jwt_settings("event-service"),
    )
    canonical = Path(__file__).parents[4] / "contracts" / "event-service.yaml"
