    ...
    try:
        with get_engine(settings).connect() as connection:
            row = connection.execute(
                text(
                    "SELECT to_regclass('booking.bookings'), "
                    "to_regclass('booking.booking_items'), "
                    "to_regclass('booking.idempotency_records'), "
                    "to_regclass('booking.booking_audit'), "
                    "to_regclass('booking.outbox_events'), "
                    "to_regclass('booking.alembic_version')"
                )
            ).one()
