...
Quy uoc dat ten bien moi truong: tien to CUSTOMER_ (giong SEAT_ cua
seat-inventory-service, IDENTITY_ cua identity-service) de tranh dung
do khi nhieu service chay chung mot may.
"""
...
class Settings:
    ...
    def from_environment(cls) -> Settings:
        ...
        return cls(
            ...
            db_pool_size=_integer("CUSTOMER_DB_POOL_SIZE", 10, minimum=1, maximum=100),
            db_max_overflow=_integer(
                "CUSTOMER_DB_MAX_OVERFLOW", 10, minimum=0, maximum=100
            ),
            sql_echo=os.getenv("CUSTOMER_SQL_ECHO", "false").strip().lower()
            in {"1", "true", "yes", "on"},
            service_jwt=ServiceJwtValidationSettings.from_environment(
                "CUSTOMER",
                audience="customer-service",
                default_allowed_subjects="booking-orchestrator,booking-service",
            ),
        )
