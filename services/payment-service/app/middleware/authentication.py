"""Service JWT authentication for internal Payment API calls.

The provider callback keeps its own HMAC scheme (app/api/v1/provider.py); a Service JWT
must never stand in for a webhook signature, and a webhook signature must never reach an
...
def require_internal_caller(
    ...
    try:
        ...
        # Never surface cryptographic detail or token material.
        raise HTTPException(
