...
def validate_manifest(report: Report) -> list[Artifact]:
    ...
    if manifest.get("sourceOfTruth") != "contracts/":
        report.error(MANIFEST_PATH, "manifest.source", "sourceOfTruth must be contracts/")
    management = set(manifest.get("managementFiles", []))
    expected_management = {"CONTRACT_STANDARD.md", "CONTRACT_REVIEW.md", "manifest.yaml"}
    if management != expected_management:
        report.error(MANIFEST_PATH, "manifest.management", f"expected {sorted(expected_management)}")
    ...
    if contract_ids != expected_ids:
        report.error(MANIFEST_PATH, "manifest.contracts", f"expected {sorted(expected_ids)}")
    for artifact in artifacts:
        if not artifact.path.is_file():
            report.error(artifact.path, "manifest.missing", "declared runtime artifact is missing")
...
def validate_catalog_shape(report: Report) -> None:
    forbidden_directories = ["common", "openapi", "soap", "events", "webhooks", "websocket"]
    for name in forbidden_directories:
        path = CONTRACTS_ROOT / name
        if path.exists():
            report.error(path, "catalog.legacy-layout", "legacy runtime contract directory must be removed")
    duplicates = [
        CONTRACTS_ROOT.parent / "services/identity-service/contracts/identity-service.yaml",
        CONTRACTS_ROOT.parent / "services/seat-inventory-service/contracts/seat-inventory.wsdl",
        CONTRACTS_ROOT.parent / "services/seat-inventory-service/contracts/seat-inventory.xsd",
    ]
    for path in duplicates:
        if path.exists():
            report.error(path, "catalog.duplicate", "service-local runtime contract duplicates the canonical root")
...
def validate_openapi_components(report: Report, path: Path, document: dict[str, Any]) -> None:
    components = document.get("components", {})
    schemes = components.get("securitySchemes", {})
    expected_schemes = {"UserJwt", "ServiceJwt", "WebhookHmac"}
    if not expected_schemes.issubset(schemes):
        report.error(path, "openapi.security-schemes", f"missing {sorted(expected_schemes - set(schemes))}")
    ...
    if set(error.get("required", [])) != {"correlationId", "error"}:
        report.error(path, "openapi.error", "ErrorResponse must require correlationId and error")
    detail = error.get("properties", {}).get("error", {})
    if set(detail.get("required", [])) != {"code", "message", "retryable"}:
        report.error(path, "openapi.error-detail", "error must require code, message and retryable")
    money = schemas.get("Money", {})
    if set(money.get("required", [])) != {"amountMinor", "currency"}:
        report.error(path, "openapi.money", "Money must require amountMinor and currency")
...
def validate_openapi_operations(
    ...
    if not {"/health/live", "/health/ready"}.issubset(document.get("paths", {})):
        report.error(path, "openapi.health", "both /health/live and /health/ready are required")
    ...
    for method, route, operation in operations(document):
        ...
        if not isinstance(operation_id, str) or not operation_id:
            report.error(path, "openapi.operation-id", f"{method} {route} has no operationId")
        elif operation_id in global_ids:
            report.error(path, "openapi.operation-id", f"duplicate {operation_id}; first in {global_ids[operation_id]}")
        else:
            global_ids[operation_id] = path
        refs = parameter_refs(operation)
        for name in ("CorrelationId", "Traceparent"):
            expected = f"#/components/parameters/{name}"
            if expected not in refs:
                report.error(path, "openapi.trace-headers", f"{method} {route} missing {name}")
        mutation = method in {"POST", "PUT", "PATCH", "DELETE"} and route not in MUTATION_EXCLUSIONS
        if mutation and not route.startswith("/health/"):
            if "#/components/parameters/IdempotencyKey" not in refs:
                report.error(path, "openapi.idempotency", f"{method} {route} missing Idempotency-Key")
        ...
        if not any(str(code).startswith("2") for code in responses):
            report.error(path, "openapi.success", f"{method} {route} has no 2xx response")
        if not route.startswith("/health/") and not any(str(code).startswith(("4", "5")) for code in responses):
            report.error(path, "openapi.error-response", f"{method} {route} has no error response")
        effective_security = operation.get("security", document.get("security"))
        if effective_security is None:
            report.error(path, "openapi.security", f"{method} {route} has no security decision")
        elif isinstance(effective_security, list):
            for requirement in effective_security:
                if isinstance(requirement, dict):
                    unknown = set(requirement) - set(schemes)
                    if unknown:
                        report.error(path, "openapi.security", f"unknown schemes {sorted(unknown)}")
...
def validate_openapi(
    ...
    for artifact in artifacts:
        ...
        if len(servers) != 1 or not isinstance(servers[0], dict):
            report.error(artifact.path, "openapi.server", "exactly one local server is required")
        else:
            parsed = urlsplit(str(servers[0].get("url", "")))
            if parsed.hostname != "localhost" or parsed.port != artifact.port:
                report.error(artifact.path, "openapi.port", f"server must use localhost:{artifact.port}")
            if parsed.username or parsed.password:
                report.error(artifact.path, "openapi.credentials", "server URL must not contain credentials")
        for ref in walk_refs(document):
            if not ref.startswith("#"):
                report.error(artifact.path, "openapi.external-ref", f"external $ref is forbidden: {ref}")
                continue
            try:
        ...
        examples = sum(1 for item in walk_mappings(document) if "example" in item or "examples" in item)
        if examples == 0:
            report.error(artifact.path, "openapi.examples", "at least one explicit example is required")
        for name in ("ErrorResponse", "Money", "HealthStatus"):
            ...
            try:
                ...
                if "example" in schema:
                    validator_class(schema, format_checker=format_checker).validate(schema["example"])
        ...
        for mapping in walk_mappings(document):
            if mapping.get("format") == "date-time" and "pattern" not in mapping:
                report.error(artifact.path, "openapi.utc", "date-time schema must constrain UTC")
    report.counts["operationIds"] = len(global_ids)


def validate_asyncapi(report: Report, validator_class: Any, format_checker: Any) -> None:
    ...
    if server.get("protocol") not in {"ws", "wss"} or server.get("host") != "localhost:8008":
        report.error(path, "asyncapi.server", "local WebSocket server must use port 8008")
    ...
    for ref in walk_refs(document):
        if not ref.startswith("#"):
            report.error(path, "asyncapi.external-ref", f"external $ref is forbidden: {ref}")
    ...
    if set(operations_found) != {"receiveRealtimeClientMessage", "sendRealtimeStatus"}:
        report.error(path, "asyncapi.operations", "receive and send operations are required")
    ...
    if len(document.get("x-close-codes", [])) < 5:
        report.error(path, "asyncapi.close-codes", "stable close-code registry is required")
    report.counts["asyncapiMessages"] = len(messages)


def validate_event_schema(report: Report, validator_class: Any, format_checker: Any) -> None:
    ...
    for ref in walk_refs(schema):
        if not ref.startswith("#"):
            report.error(path, "json-schema.external-ref", f"external $ref is forbidden: {ref}")
            continue
        try:
    ...
    for item in event_refs:
        if not isinstance(item, dict) or not isinstance(item.get("$ref"), str):
            report.error(path, "json-schema.oneOf", "event variants must be local $ref entries")
            continue
        ref = item["$ref"]
        ...
        examples = event_schema.get("examples", []) if isinstance(event_schema, dict) else []
        if not examples:
...
def validate_http_examples(report: Report, validator_class: Any, format_checker: Any) -> None:
    ...
    for entry in index:
        ...
        valid_schema_selector = isinstance(schema_name, str) ^ isinstance(schema_ref, str)
        if not isinstance(contract_name, str) or not isinstance(filename, str) or not valid_schema_selector:
        ...
        if instance is None:
            continue
        ref = schema_ref if isinstance(schema_ref, str) else f"#/components/schemas/{schema_name}"
        wrapper = {
        ...
        try:
            ...
            report.error(index_path.parent / filename, "example.http", f"{contract_name}{ref}: {exc}")
        count += 1
...
def validate_soap(report: Report, etree: Any) -> None:
    ...
    expected = {
        "GetSeatMap", "CheckAvailability", "ReserveSeats", "GetReservation",
        "ExtendReservation", "ConfirmSeats", "ReleaseSeats", "ExpireReservations",
    }
    found = {node.get("name") for node in root.xpath("./wsdl:portType/wsdl:operation", namespaces=ns)}
    if found != expected:
        report.error(wsdl_path, "soap.operations", f"expected {sorted(expected)}, got {sorted(found)}")
    ...
    addresses = root.xpath("./wsdl:service/wsdl:port/soap:address/@location", namespaces=ns)
    if addresses != ["http://localhost:8003/soap"]:
        report.error(wsdl_path, "soap.port", "SOAP address must use localhost:8003/soap")
    text = wsdl_path.read_text(encoding="utf-8")
    if "SeatServiceFault" not in text or "Service JWT" not in text:
        report.error(wsdl_path, "soap.security-error", "Service JWT documentation and SeatServiceFault are required")
    ...
    for example in examples:
        ...
        except (OSError, ValueError, etree.XMLSyntaxError, etree.DocumentInvalid) as exc:
    ...
    if request_names != expected_requests:
        report.error(CONTRACTS_ROOT / "examples/soap", "soap.examples", "one request example per operation is required")
