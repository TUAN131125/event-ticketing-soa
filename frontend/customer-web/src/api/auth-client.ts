...
export class ApiError extends Error {
  ...
  constructor(status: number, payload: AuthErrorPayload | undefined, fallback = 'Request failed') {
    ...
    this.name = 'ApiError';
    this.status = status;
    this.code = payload?.error?.code ?? (status === 401 ? 'UNAUTHENTICATED' : status === 403 ? 'FORBIDDEN' : 'REQUEST_FAILED');
...
export class AuthClient {
  ...
  constructor(options: AuthClientOptions = {}) {
    const transport = options.transport ?? (import.meta.env.VITE_AUTH_TRANSPORT as AuthTransport | undefined) ?? 'direct';
  ...
  private async request<T>(path: string, init: RequestInit = {}, retryOn401 = true): Promise<T> {
    if (!this.baseUrl) throw new ApiError(503, { error: { code: 'SERVICE_UNAVAILABLE', message: 'Identity service is not configured.', retryable: true } });
    ...
    if ((path.endsWith('/refresh') || path.endsWith('/logout')) && this.csrfToken()) headers.set('X-CSRF-Token', this.csrfToken() as string);
    let response: Response;
    try { response = await this.fetchImpl(`${this.baseUrl}${path}`, { ...init, headers, credentials: 'include' }); }
    catch { throw new ApiError(503, { error: { code: 'SERVICE_UNAVAILABLE', message: 'Authentication service is temporarily unavailable.', retryable: true } }); }
    if (response.status === 401 && retryOn401 && !path.endsWith('/refresh') && !path.endsWith('/login')) {
      try {
        await this.refresh();
    ...
    if (!response.ok) {
      ...
      try { payload = readJson(await response.json()); } catch { /* response may be empty */ }
  ...
  async register(email: string, password: string): Promise<User> {
    return this.request<User>('/auth/register', { method: 'POST', body: JSON.stringify({ email: email.trim().toLowerCase(), password }) }, false);
  }

  async login(email: string, password: string): Promise<TokenResponse> {
    return this.storeSession(await this.request<TokenResponse>('/auth/login', { method: 'POST', body: JSON.stringify({ email: email.trim().toLowerCase(), password }) }, false));
  }
  ...
  async logout(): Promise<void> {
    try { await this.request<void>('/auth/logout', { method: 'POST' }, false); } finally { this.clearSession(); }
  }

  async changeRole(userId: string, role: Role, action: 'ASSIGN' | 'REVOKE'): Promise<unknown> {
    return this.request(`/admin/users/${encodeURIComponent(userId)}/roles`, { method: 'POST', body: JSON.stringify({ role, action }) });
  }
}
