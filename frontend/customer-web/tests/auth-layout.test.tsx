  ...
  await act(async () => {
    view = render(
      <MemoryRouter initialEntries={[`/${page}`]}>
        <AuthProvider>{page === 'login' ? <LoginPage /> : <RegisterPage />}</AuthProvider>
      </MemoryRouter>,
    );
  });
  return view;
...
describe('customer authentication layout', () => {
  ...
  it('links validation errors to both inputs without changing their control class', async () => {
    ...
    expect(email).toHaveAttribute('aria-describedby', 'customer-auth-email-error');
    expect(password).toHaveAttribute('aria-describedby', 'customer-auth-password-error');
