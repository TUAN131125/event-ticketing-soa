...
const identityBase = () =>
  import.meta.env.VITE_IDENTITY_API_URL || import.meta.env.VITE_ESB_API_URL || '';
...
const mapUser = (value: RawUser): User => ({
  id: value.id ?? value.userId ?? '',
  email: value.email,
  displayName: value.displayName,
  roles: value.roles ?? (value.role ? [value.role] : []),
  status: value.status,
});
...
const mapSession = (value: AuthPayload): AuthSession => {
  ...
  if (!accessToken || !value.user)
    throw new ApiError('Authentication response was incomplete', { code: 'INVALID_AUTH_RESPONSE' });
...
export class AuthClient {
  ...
  async login(input: { email: string; password: string }): Promise<AuthSession> {
    return mapSession(
      await request<AuthPayload>(this.base, `${authPrefix}/login`, { method: 'POST', body: input }),
    );
  }
  async refresh(): Promise<AuthSession> {
    const csrf = sessionStorage.getItem(csrfKey);
    return mapSession(
      await request<AuthPayload>(this.base, `${authPrefix}/refresh`, {
        method: 'POST',
        body: {},
  ...
  async logout(accessToken: string): Promise<void> {
    const csrf = sessionStorage.getItem(csrfKey);
    await request(this.base, `${authPrefix}/logout`, {
      method: 'POST',
      body: {},
      accessToken,
      headers: csrf ? { 'X-CSRF-Token': csrf } : undefined,
  ...
  async assignRole(
    ...
    await request(this.base, `/admin/users/${encodeURIComponent(userId)}/roles`, {
      method: 'POST',
      body: { role, action: action.toUpperCase() },
      accessToken,
    });
  }
}
