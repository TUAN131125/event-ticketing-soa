...
export function IconButton({ label, size = 'md', className, children, ...props }: IconButtonProps) {
  return (
    <Button
      ...
      size={size}
      aria-label={label}
      className={cx('ui-icon-button', size === 'sm' && 'ui-icon-button--sm', className)}
    >
      {children}
    </Button>
  );
...
export interface LinkProps extends React.AnchorHTMLAttributes<HTMLAnchorElement>, ClassName {
  muted?: boolean;
}
...
export interface SelectProps extends React.SelectHTMLAttributes<HTMLSelectElement>, ClassName {
...
export interface FieldErrorProps extends ClassName {
  children?: React.ReactNode;
  id?: string;
}
export function FieldError({ children, id, className }: FieldErrorProps) {
  return children ? (
    <p id={id} className={cx('ui-field__error', className)} role="alert">
      {children}
    </p>
  ) : null;
}
...
export interface AlertProps extends ClassName {
  tone?: AlertTone;
...
export function Alert({ tone = 'info', title, children, onClose, className }: AlertProps) {
  ...
  return (
    ...
    >
      ...
      {onClose ? (
        <IconButton label="Dismiss" size="sm" variant="ghost" onClick={onClose}>
...
export type BadgeTone =
  'neutral' | 'brand' | 'success' | 'warning' | 'danger' | 'info' | 'information';
export function Badge({
  tone = 'neutral',
  children,
  className,
}: ClassName & { tone?: BadgeTone; children: React.ReactNode }) {
  return <span className={cx('ui-badge', `ui-badge--${tone}`, className)}>{children}</span>;
}

export interface CardProps extends React.HTMLAttributes<HTMLDivElement>, ClassName {
  padded?: boolean;
}
export function Card({ padded = false, className, ...props }: CardProps) {
  return <div className={cx('ui-card', padded && 'ui-card--padded', className)} {...props} />;
}
export function CardHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cx('ui-card__header', className)} {...props} />;
}
export function CardTitle({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) {
  return <h2 className={cx('ui-card__title', className)} {...props} />;
}
...
export interface ConfirmationDialogProps extends Omit<DialogProps, 'footer'> {
  confirmLabel?: string;
  cancelLabel?: string;
  onConfirm: () => void;
  tone?: 'primary' | 'danger';
...
export interface DrawerProps extends ClassName {
  open: boolean;
  title: React.ReactNode;
  onClose: () => void;
  children: React.ReactNode;
...
export function Drawer({ open, title, onClose, children, side = 'right', className }: DrawerProps) {
  ...
  return (
    <div
      className={cx('ui-overlay', 'ui-drawer-overlay', side === 'left' && 'ui-drawer--left')}
      role="presentation"
      onMouseDown={(event) => {
        if (event.target === event.currentTarget) onClose();
      }}
    >
      <aside
        ...
        role="dialog"
        aria-modal="true"
        aria-label={typeof title === 'string' ? title : 'Navigation drawer'}
      >
        <header className="ui-dialog__header">
          <h2 className="ui-dialog__title">{title}</h2>
          <IconButton label="Close drawer" variant="ghost" onClick={onClose}>
            <X size={18} aria-hidden="true" />
          </IconButton>
        </header>
        <div className="ui-dialog__body">{children}</div>
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const dismiss = React.useCallback(
    (id: string) => setMessages((current) => current.filter((item) => item.id !== id)),
    [],
  );
...
export function Pagination({
  ...
  return (
    <nav className={cx('ui-pagination', className)} aria-label="Pagination">
      ...
      <Button variant="outline" size="sm" disabled={page >= count} onClick={() => change(page + 1)}>
        Next
...
export function Skeleton({
  ...
  return (
    <span className={cx('ui-skeleton', className)} style={{ width, height }} aria-hidden="true" />
  );
}
export function Spinner({ label = 'Loading', className }: { label?: string; className?: string }) {
  return (
    <span className={cx('ui-spinner-wrap', className)} role="status">
...
export function EmptyState({
  ...
  return (
    <div className={cx('ui-state', className)}>
      ...
      {(message ?? description) ? (
        <p className="ui-state__message">{message ?? description}</p>
      ) : null}
      {action}
    </div>
  );
...
export function ErrorState({
  ...
  return (
    <div className={cx('ui-state', className)} role="alert">
      <div className="ui-state__icon" aria-hidden="true">
        <AlertCircle size={22} />
      </div>
      <h2 className="ui-state__title">{title}</h2>
      <p className="ui-state__message">{message ?? description ?? 'Please try again.'}</p>
      {action}
    </div>
  );
}
export interface RetryStateProps extends StateProps {
  onRetry?: () => void;
}
  ...
  return (
    <ErrorState
      ...
      message={
        message ??
        description ??
        'We cannot reach the ticketing service right now. Your data has not been changed. Please try again shortly.'
      }
      action={
        action ??
        (onRetry ? (
...
export function UnauthorizedState({ action, title, message, description, className }: StateProps) {
  return (
    <ErrorState
      className={className}
      title={title ?? 'Sign in required'}
      message={message ?? description ?? 'Sign in to continue and view this information.'}
      action={action}
    />
  );
}
export function ForbiddenState({ action, title, message, description, className }: StateProps) {
  return (
    <ErrorState
      className={className}
      title={title ?? 'You do not have access'}
      message={message ?? description ?? 'Your account is not permitted to view this page.'}
      action={action}
    />
  );
}
export function NotFoundState({
  ...
  return (
    <ErrorState
      className={className}
      title={title}
      message={message ?? description ?? 'The page or resource you requested could not be found.'}
      action={action}
    />
  );
}
  ...
  return (
    <div className={cx('ui-table-wrap', className)}>
      <table className="ui-table">
        <caption className="ui-visually-hidden">{caption ?? 'Data table'}</caption>
        <thead>
          <tr>
            {columns.map((column) => (
